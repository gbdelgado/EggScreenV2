...
{
    lv_subject_t boost;
    lv_subject_init_int(&boost, MIN_BOOST);
    this->boost = boost;
    ...
    lv_arc_bind_value(arc, &this->boost);
    ...
    lv_subject_add_observer(&this->boost, handle_boost_change, this);
...
void TurboGauge::set_value(int value)
{
    lv_subject_set_int(&this->boost, value);
}
...
{
    TurboGauge *self = static_cast<TurboGauge *>(var);
    if (!self)
    {
        return;
    }
